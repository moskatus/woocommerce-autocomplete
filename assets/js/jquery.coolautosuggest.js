	...
	function autosuggest(callBackUrl, textField){
											...
											for(i=0;i<arr.length;i++){
												...
												if(me.description==true){
													if(arr[i].description!=undefined){
														desc='<div class="description">R$ ' + arr[i].description + '</div>';
													}
												}

												html+='<a href="'+ arr[i].id +'"> <div id="' + suggestRow + (i+1) + '" class="' + cssClass + '"' + id_field + ' seq_id="' + i + '" >' + thumb + '<div class="suggestion_title">' + arr[i].data.replace(new RegExp('(' + me.textField.val() + ')', 'gi'), "<b>$1</b>") + '</div>' + desc + '</div>';
											}
		...
		this.show=function(height){
			this.holder.css({
				...
				"top":this.textField.position().top + this.textField.height() + 19 + "px",
				"height":height + "px"
			});
